...
- never append `--yolo`, `--dangerously-skip-permissions`,
  ...
  `--allow-all-urls`, `--allow-all`, `--auto`, Cursor `--force`, `--trust`,
  `--approve-mcps`, `--sandbox`, or equivalent;
...
values remain unread. The Cursor status exception reduces provider output to
