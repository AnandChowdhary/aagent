...
| 10 | Cursor `agent` is not `aagent` | [`test_discovery.sh`](../tests/test_discovery.sh) and [`test_discovery.ps1`](../tests/test_discovery.ps1) separately discover Cursor's `agent`, reject wrapper recursion/self-paths, and retain distinct `cursor` and `aagent` identities. Introspection exposes Cursor only as a planned Tier 2 adapter. | Pass |
