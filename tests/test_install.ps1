...
try {
    ...
    $env:AAGENT_EXPECTED_VERSION = "0.1.1"
    ...
    Assert-Equal $version "aagent 0.1.1" "installed PowerShell version differs"
    ...
    Assert-Equal $version "aagent 0.1.1" "Invoke-Expression install did not execute"
    $env:INSTALL_DIR = $installDir

    [IO.File]::WriteAllText($targetPath, "old-install", $utf8)
    & $installScript *> $null
    $version = (& pwsh -NoLogo -NoProfile -File $targetPath --version | Out-String).Trim()
    Assert-Equal $version "aagent 0.1.1" "existing PowerShell install was not replaced"
    ...
    $version = (& pwsh -NoLogo -NoProfile -File $targetPath --version | Out-String).Trim()
    Assert-Equal $version "aagent 0.1.1" "checksummed remote PowerShell install failed"
